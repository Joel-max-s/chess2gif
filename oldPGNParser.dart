...
class PGNParser {
  ...
  List<List<String>> parsePGN() {
    ...
    //RegExp testSplit = new RegExp(r'(NBKRQ]?[a-h]?[1-8]?[\-x]?[a-h][1-8](?:=?[nbrqkNBRQK])?|[PNBRQK]?@[a-h][1-8]|--|Z0|0000|@@@@|O-O(?:-O)?|0-0(?:-0)?)|(\{.*)|(;.*)|(\$[0-9]+)|(\()|(\))|(\*|1-0|0-1|1/2-1/2)|([\?!]{1,2})');
    RegExp secondSplit = new RegExp(r'[a-zA-Z][a-zA-Z0-9+-]+');

    String pgn = File('kjj.pgn').readAsStringSync().replaceAll('\n', ' ');
    pgn += ' ';
